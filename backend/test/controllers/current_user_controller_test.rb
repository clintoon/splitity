...
class CurrentUserControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'github_app_installations returns unauthorized when auth header is invalid' do
    Rails.application.credentials.stubs(:github).returns({ client_id: 'client_id', client_secret: 'client_secret' })
    ...
    get('/v1/current_user/github_app_installations', { headers: { 'HTTP_AUTHORIZATION': 'Bearer abc123' } })

    assert_response :unauthorized
  end

  test 'github_app_installations returns unauthorized when gh access token is invalid' do
    Rails.application.credentials.stubs(:github).returns({ client_id: 'client_id', client_secret: 'client_secret' })
    ...
    get('/v1/current_user/github_app_installations', { headers: { 'HTTP_AUTHORIZATION': "Bearer #{invalid_auth_token}" } })

    assert_response :unauthorized
  end

  test 'github_app_installations returns success when auth token is valid' do
    Rails.application.credentials.stubs(:github).returns({ client_id: 'client_id', client_secret: 'client_secret' })
    ...
    GithubService.any_instance.stubs(:current_user).returns({ id: 123, login: 'clintoon' })
    GithubService.any_instance.stubs(:app_installations_for_user).returns([{ installation_id: 1, account_id: 1 }, { installation_id: 2, account_id: 2 }])

    get('/v1/current_user/github_app_installations', { headers: { 'HTTP_AUTHORIZATION': "Bearer #{valid_auth_token}" } })

    parsed_resp = JSON.parse(@response.body)

    assert_response :success
    assert_equal parsed_resp['installations'], [{ 'installation_id' => 1, 'account_id' => 1 }, { 'installation_id' => 2, 'account_id' => 2 }]
  end
end
