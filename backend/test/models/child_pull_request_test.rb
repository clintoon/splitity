...
class ChildPullRequestTest < ActiveSupport::TestCase
  test 'valid' do
    create(:split_pull_request_job, id: 1)
    child_pr = build(:child_pull_request, split_pull_request_job_id: 1)

    assert child_pr.valid?
  end

  test 'invalid when with split_pull_request_job_id is missing' do
    child_pr = build(:child_pull_request, split_pull_request_job_id: nil)
  ...
  test 'invalid when child_pr_id is missing' do
    create(:split_pull_request_job, id: 1)
    child_pr = build(:child_pull_request, split_pull_request_job_id: 1, child_pr_id: nil)
  ...
  test 'invalid when the coresponding split pull request job does not exist' do
    child_pr = build(:child_pull_request, split_pull_request_job_id: 1, child_pr_id: 1)
